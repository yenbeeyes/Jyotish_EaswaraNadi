...
if mode == "By Lagna":
    ...
    for _, row in filtered_df.iterrows():
        with st.expander(f"📊 Chart No: {int(row['ChartNo'])}"):
            st.markdown(f"**Sun:** {safe(row['Sun'])} | **Moon:** {safe(row['Moon'])} | **Mars:** {safe(row['Mars'])}")
            st.markdown(f"**Mercury:** {safe(row['Mercury'])} | **Jupiter:** {safe(row['Jupiter'])} | **Venus:** {safe(row['Venus'])}")
            st.markdown(f"**Saturn:** {safe(row['Saturn'])} | **Rahu:** {safe(row['Rahu'])} | **Ketu:** {safe(row['Ketu'])}")
