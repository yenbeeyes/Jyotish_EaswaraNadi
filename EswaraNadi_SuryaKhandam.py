...
def display_verse_block(verse_id):
    verses_df = load_verses(DEFAULT_VERSES_CSV)
    verses_df["VerseID"] = verses_df["VerseID"].astype(str).str.strip()
...
if not verse_row.empty:
        ...
        st.info(f"📜 Verse not available for {verse_id}.")# Sidebar controls

        st.markdown("🔍 **Verse Missing**", unsafe_allow_html=True)
    ...
    if filtered_df.empty:
        ...
        for _, row in filtered_df.iterrows():
           with st.expander(f"📊 {row['Lagna']} Lagna — Chart ID: {row['VerseID']}"):
                ...
                st.caption(f"🖼️ ImagePath: {row.get('ImagePath', 'Missing')}")
