...
# File paths
CHART_CSV = "Chandra_Khandam.csv"  # adjust if needed
    ...
    if not verse_row.empty:
        ...
        if editable:
            ...
            if st.button(f"💾 Save Verse {verse_id}"):
                ...
                verses_df.to_csv(f"Surya_Verses_{selected_lagna}.csv", index=False, encoding='utf-8')
            ...
            with st.expander(f"📊 {row['Lagna']} Lagna — Chart ID: {row['VerseID']}"):
                ...
                if image_path:
                    image_url = image_path.replace(
                        "Jyotish_EaswaraNadi/images/",
                        "https://raw.githubusercontent.com/yenbeeyes/Jyotish_EaswaraNadi/main/images/"
                    )
...
# Mode: ALL Charts (uses all combined chart data but not verses)
