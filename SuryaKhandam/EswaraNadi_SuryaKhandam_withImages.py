...
def load_verse_data_by_lagna(lagna):
    filename = f"SuryaKhandam/Surya_Verses_{lagna}.csv"
    try:
...
# Load concise interpretations for Surya Khandam (optional)
@st.cache_data
def load_interpretations():
    try:
        ...
        interp_dict = dict(zip(interp_df["VerseID"], interp_df["Concise Interpretation"]))
        return interp_dict
    except Exception as e:
...
def display_verse_block(verse_id, verses_df, editable=False):
    ...
    if not verse_row.empty:
        ...
        interpretation = interpretation_lookup.get(verse_id, "")
        if interpretation:
